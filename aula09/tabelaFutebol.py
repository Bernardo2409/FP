...
pontos = lambda reg: 3 * int(reg[V]) + int(reg[E])

diferenca_de_golos = lambda reg: int(reg[GM]) - int(reg[GS])
...
def main():
    ...
    tab = sorted(tabela, key=diferenca_de_golos,reverse=True  )
    printTabela(tab)
    ...
    tab = sorted(tabela, key=lambda reg: (pontos(reg), diferenca_de_golos(reg)), reverse=True)
    printTabela(tab)
